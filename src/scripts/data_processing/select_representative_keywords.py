...
def group_similar_keywords(keywords, min_group_size=3):
    """Group similar keywords based on prefix matching."""
    groups = []
    current_group = []
...
def process_keywords(input_file, output_file):
    """Process keywords to select representatives."""
...
if __name__ == "__main__":
    input_path = os.path.join(os.path.dirname(__file__), '../../../data/keywords/unique_keywords_with_location.csv')
    output_path = os.path.join(os.path.dirname(__file__), '../../../data/keywords/uniquerepresentative_keywords_with_location.csv')
